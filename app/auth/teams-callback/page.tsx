    ...
    const exchangeCode = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn("Teams connected but refreshUser failed:", refreshErr);
        }
        setTimeout(() => router.push("/dashboard?tab=integrations"), 1500);
      } catch (err: unknown) {
        console.error("Teams callback exchange exception:", err);
        setStatus("error");
        setErrorMessage(err instanceof Error ? err.message : "An error occurred during Teams sync.");
      }
    };
    ...
    <div className="w-full max-w-sm glass-premium rounded-3xl p-8 relative z-10 text-center space-y-6 border border-white/5 shadow-2xl">
      {status === "loading" && (
        <div className="py-6 space-y-4">
          <div className="w-14 h-14 rounded-full border-2 border-dashed border-indigo-500 animate-spin flex items-center justify-center mx-auto">
            <div className="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center">
              <span className="w-2 h-2 rounded-full bg-indigo-400"></span>
            </div>
          </div>
          <div>
            <h3 className="text-sm font-bold text-white">Connecting Microsoft Teams</h3>
      ...
      {status === "success" && (
        <div className="py-6 space-y-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Teams Synced Successfully</h3>
...
export default function TeamsCallbackPage() {
  return (
    <div className="min-h-screen text-slate-200 bg-[#030712] relative overflow-hidden font-sans flex items-center justify-center px-6">
      <div className="absolute top-[30%] left-[30%] w-[40%] h-[40%] rounded-full bg-indigo-900/10 filter blur-3xl pointer-events-none animate-pulse" />
      ...
      <Suspense
        fallback={
          <div className="w-full max-w-sm glass-premium rounded-3xl p-8 text-center border border-white/5 shadow-2xl">
            ...
            <p className="text-xs text-slate-500 animate-pulse">Initializing Teams callback...</p>
          </div>
        }
      >

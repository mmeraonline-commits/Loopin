...
function SlackCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      setStatus("error");
      setErrorMessage(errorParam || "Access denied by Slack.");
      return;
    }
    ...
    const exchangeCode = async () => {
      ...
      try {
        ...
        console.error("Slack callback exchange exception:", err);
        setStatus("error");
        setErrorMessage(err instanceof Error ? err.message : "An error occurred during Slack sync.");
      }
    };
    ...
    <div className="w-full max-w-sm glass-premium rounded-3xl p-8 relative z-10 text-center space-y-6 border border-white/5 shadow-2xl">
      {status === "loading" && (
        <div className="py-6 space-y-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Connecting Slack Workspace</h3>
            <p className="text-[11px] text-slate-500 mt-1">Exchanging secure credentials with Slack OAuth...</p>
          </div>
        </div>
      )}
      ...
      {status === "success" && (
        <div className="py-6 space-y-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Slack Synced Successfully</h3>
      ...
      <Suspense
        fallback={
          <div className="w-full max-w-sm glass-premium rounded-3xl p-8 text-center border border-white/5 shadow-2xl">
            ...
            <p className="text-xs text-slate-500 animate-pulse">Initializing Slack callback...</p>
          </div>
        }
      >

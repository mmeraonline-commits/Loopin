    ...
    const exchangeCode = async () => {
      ...
      try {
        ...
        try {
          ...
          // Integration is already saved — don't fail the whole connect on profile refresh.
          console.warn("Google Calendar connected but refreshUser failed:", refreshErr);
        }
        setTimeout(() => {
        ...
        setStatus("error");
        setErrorMessage(err instanceof Error ? err.message : "An error occurred during account sync.");
      }
    };
    ...
    <div className="w-full max-w-sm glass-premium rounded-3xl p-8 relative z-10 text-center space-y-6 border border-white/5 shadow-2xl">
      {status === "loading" && (
        <div className="py-6 space-y-4">
          <div className="w-14 h-14 rounded-full border-2 border-dashed border-sky-500 animate-spin flex items-center justify-center mx-auto">
            <div className="w-8 h-8 rounded-full bg-sky-500/20 flex items-center justify-center">
              <span className="w-2 h-2 rounded-full bg-sky-400"></span>
            </div>
          </div>
          <div>
            <h2 className="text-lg font-semibold text-white">Connecting Google Calendar</h2>
      ...
      {status === "success" && (
        <div className="py-6 space-y-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-white">Calendar connected</h2>

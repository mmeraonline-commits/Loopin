...
function CalendlyCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      setStatus("error");
      setErrorMessage(errorParam || "Access denied by Calendly.");
      return;
    }
    if (!code) {
      setStatus("error");
      setErrorMessage("No authorization code received from Calendly.");
      return;
    }
    ...
    const exchangeCode = async () => {
      ...
      try {
        ...
        if (!response.ok || data.error) throw new Error(data.error || "Calendly OAuth failed.");
        setStatus("success");
        await refreshUser();
        setTimeout(() => router.push("/dashboard?tab=integrations"), 1500);
      } catch (err: unknown) {
        setStatus("error");
        setErrorMessage(err instanceof Error ? err.message : "Calendly sync failed.");
      }
    };
    exchangeCode();
    ...
    <div className="w-full max-w-sm glass-premium rounded-3xl p-8 relative z-10 text-center space-y-6 border border-white/5 shadow-2xl">
      {status === "loading" && (
        <div className="py-6 space-y-4">
          <div className="w-14 h-14 rounded-full border-2 border-dashed border-blue-500 animate-spin mx-auto" />
          <h3 className="text-sm font-bold text-white">Connecting Calendly</h3>
      ...
      {status === "success" && (
        <div className="py-6 space-y-4">
          ...
          <h3 className="text-sm font-bold text-white">Calendly Connected</h3>
